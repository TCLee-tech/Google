...
Package server applications as containers
...
#### Task 1. Containerizing the dedicated game server
DGS pattern:
...
#### Task 2. Creating a dedicated game server binaries container image
1. Create a VM
...
2. Install kubectl and docker on VM
  ...
  - install docker dependencies, official GPG keys (to validate integrity of Linux package to be downloaded), add stable Docker repo, update package cache, install Docker-ce, run as non-root user.
3. Generate container image
  ...
  - write [Dockerfile](https://docs.docker.com/engine/reference/builder/)
...
#### Create asset disk
Game assets put on a read-only [persistent disk](https://cloud.google.com/compute/docs/disks/#pdspecs), attached to multiple VM instances running DGS container.
1. Store a zone ID in an environmental variable
```
region=us-east1
zone_1=${region}-b
...
6. Format the persistent disk with `mkfs.ext4` [command](https://manpages.ubuntu.com/manpages/xenial/man8/mkfs.8.html)
  ...
  - confirm device ID for openarena-assets disk
...
Two factors to consider:
  1. The largest number of concurrent DGS pods you plan to run. [Kubernetes has a max number of nodes per cluster](https://kubernetes.io/docs/setup/best-practices/cluster-large/). 1 node is 1 physical or virtual machine. Each machine can have multi-core (vCPUs). Each vCPU can run one or more DGS. A multi-core VM can support more DGS than a single-core VM.
  2. VM instance gradularity. The simplest way for resources to be added/removed is in increments of a single VM of the type chosen during cluster creation.
...
Typical DGS do not need write access to game assets
...
- each DGS instance only lasts length of a single game match.
...
5. Customize file with cluster's environment variables
  - get name of base instance `gcloud compute instance-groups managed list`
...
Scale up nodes
  ...
  - use [resource requests and limits for pods](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#resource-requests-and-limits-of-pod-and-container) to specify resource (i.e. CPU and memory) for a DGS (i.e. pod). Example: 1 vCPU and 500MB memory.
